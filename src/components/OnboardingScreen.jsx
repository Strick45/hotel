...
const OnboardingScreen = ({ onComplete, isChatOnly = false }) => {
  ...
  const checkUser = async (phone) => {
    try {
      const response = await fetch(`http://${BACKEND_URL}/check-user`, {
        method: 'POST',
        headers: {
    ...
    const sendOtp = async (phone) => {
      try {
        const response = await fetch(`http://${BACKEND_URL}/send-otp`, {
          method: 'POST',
          headers: {
            'accept': 'application/json',
            'Content-Type': 'application/json',
    ...
    const verifyOtp = async (phone, otp) => {
      try {
        const response = await fetch(`http://${BACKEND_URL}/verify-otp`, {
          method: 'POST',
          headers: {
            'accept': 'application/json',
            'Content-Type': 'application/json',
  ...
  const baseSteps = [
    ...
    { text: 'Ваш номер телефона', emoji: null, input: true, placeholder: 'Введите ваш номер телефона...' },
    ...
    { text: 'Введите SMS пароль', emoji: null, input: true, placeholder: 'Введите SMS пароль...' },
    { text: 'Давайте поговорим', emoji: null },
    { text: 'Чат с ботом', emoji: null, chat: true },
  ];


  const smsStep = { text: 'Введите SMS пароль', emoji: null, input: true, placeholder: 'Введите SMS пароль...' };

  const steps = dynamicSteps.length > 0 ? dynamicSteps : baseSteps;
          ...
          if (currentStep + 1 >= 8) {
            ...
            setTimeout(() => {
              ...
              if (steps[currentStep + 1]?.chat) {
                setTimeout(() => {
                  setIsChatMode(true);
                  setChatMessages([{ type: 'bot', text: 'Когда вы планируете приехать в Ростов?' }]);
                }, 500);
              }
            }, 1000);
          }
      ...
      <div className={`onboarding-content ${isVisible ? 'fade-in' : 'fade-out'}`}>
        ...
        {current.input && (
          <>
                ...
                if (e.key === 'Enter' && userInput.trim()) {
                  if (current.placeholder === 'Введите ваш номер телефона...') {
                    ...
                    try {
                      ...
                      if (result.exists) {
                        // User exists, go to SMS step
                        setDynamicSteps([smsStep]);
                        setCurrentStep(0);
                        // Send OTP for existing user
                        try {
                          await sendOtp(phone);
                        } catch (error) {
                          setError('Ошибка при отправке SMS. Попробуйте еще раз.');
                        }
                        setCanProceed(true);
                        setTimeout(() => {
                          setIsVisible(false);
                        ...
                        setCurrentStep(2); // Skip to "Для начала"
                        setCanProceed(true);
                        setTimeout(() => {
                          setIsVisible(false);
                          setTimeout(() => {
                  ...
                  } else if (current.placeholder === 'Введите SMS пароль...') {
                      ...
                      if (result.access_token) {
                        ...
                        setTimeout(() => {
                          ...
                          setTimeout(() => {
                            ...
                            setCanProceed(false);
                            setUserInput('');
                            if (onComplete) onComplete(); // Go to main menu after SMS
                          }, 300);
                        }, 500);
                      } else {
                        setError('Неверный SMS код. Попробуйте еще раз.');
                      }
                    } catch (error) {
                      setError('Ошибка при проверке SMS кода. Попробуйте еще раз.');
                    }
                  } else {
                    if (current.placeholder === 'Введите ваше имя...') {
                      ...
                      // Register user after age
                      try {
                        await registerUser({
                          phone_number: userData.phone,
                          name: userData.name,
                          age: parseInt(userInput.trim()),
                          gender: userData.gender || 'Мужской' // Default, will be updated in chat
                        });
                      } catch (error) {
            ...
            {checkingUser && <div className="loading-message">Проверяем пользователя...</div>}
